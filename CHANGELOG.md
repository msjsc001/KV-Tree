...
## [v1.2.1] - 2026-02-28
### 🐛 缺陷修复 (Bugfixes)
- **后台自动更新监控失灵修复**：修复了勾选「后台自动更新」后，源笔记文件发生修改时不会自动检测并更新词库的严重 Bug。根因为监控服务在新增/移除/启禁源路径时未自动重启，导致 watchdog 仍监视旧路径。
- **文件事件防抖逻辑重构**：移除底层 `AppEventHandler` 中过于激进的 1 秒初级防抖拦截（会误杀笔记软件原子保存产生的多段写入信号），全部交由 `TaskDispatcher` 的 2 秒全局智能防抖统一调度，确保零丢失。
- **批量事件处理竞态修复**：重构 `TaskDispatcher._process_dirty_batch`，对同一文件的多种事件去重后以磁盘实际存在状态 (`os.path.exists`) 为准，消除因集合无序导致"删除事件晚于创建事件执行"而意外清空缓存的隐患。
