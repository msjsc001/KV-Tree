...
- **[新] Logseq页内属性扫描**: 增强了针对Logseq用户的扫描功能，现在可以从`.md`文件内的**任意位置**（包括页头和无序列表下）提取属性的键（如 `tags::`）和值（如 `[[TODO]]`）来构建词库。
...
如果您是 Logseq 用户，现在可以利用增强的属性扫描功能，直接从笔记的**任意位置**（页眉或正文中的无序列表）提取属性来构建词条。

1.  **启用功能**:
    -   在主界面点击“**高级选项**”按钮。
    -   在弹出的窗口中，根据您的需要勾选：
        -   `页内属性键录入为词条`：会提取所有 `key::` 格式的键。
        -   `页内属性值录入为词条-带双方括号[[]]的`：会提取所有 `[[value]]` 格式的值。
    ...
    **示例:** 对于以下笔记内容：
    ...
    - 这是另一个列表项
      - 嵌套任务
        priority:: high
    ```
    勾选相应功能后，程序会提取出 `- title::`, `- status::`, `- assignee::`, `- priority::`, `- [[TODO]]`, 和 `- [[张三]]` 等词条。
...
为了从根本上解决处理大规模笔记库（数千个文件）时的性能瓶颈，v0.7 版本引入了全新的缓存机制和多线程异步架构。
...
## 更新日志
- ✔️V0.9.1 (本次更新) 增强Logseq属性扫描，实现**页内属性**的全面提取，不再局限于页头。
