...
![KVTree UI](https://raw.githubusercontent.com/L-M-Sherlock/QuickKV-KVTreeApp/main/img/app_ui.png)
...
- **简洁直观的UI**: 使用轻量级的 `Tkinter` 构建，界面简洁，操作直观，无需学习成本即可上手。
...
- **优势**:
    ...
    - **健壮性**: 无论是混合使用不同模式，还是在块内部嵌套单行，AST都能清晰地界定每个标签的作用范围，避免逻辑冲突。
    - **可维护性**: 所有解析逻辑都封装在 `ast_parser.py` 中，与UI代码完全分离。未来若想增加新的提取模式，只需在 `_extract_data` 方法中增加一个新的处理分支即可，不会影响现有功能。
...
```
.
├── ast_parser.py         # 核心：AST解析器模块
├── kv_tree_app.py        # 主程序：UI界面与应用逻辑
├── kv_tree_config.json   # 配置文件：保存源文件列表、输出路径和自定义规则
├── 测试内容.txt          # 自动化测试用例文件
└── README.md             # 本文档
```
...
1.  **修改解析逻辑**: 所有核心解析功能都集中在 `ast_parser.py` 中。如果您想修改提取规则或增加新模式，请重点关注 `_extract_data` 方法。
2.  **修改UI**: 所有界面元素都在 `kv_tree_app.py` 的 `KvTreeApp` 类中定义。
3.  **增加测试用例**: 在 `测试内容.txt` 中，按照现有格式添加新的测试案例，然后运行 `kv_tree_app.py`，测试脚本会自动执行并报告结果。这是确保代码质量的最佳实践。
